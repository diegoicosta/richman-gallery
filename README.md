...
Responsive photo gallery with a fullscreen viewer, multi-select, tags, likes and comments.
No dependencies — a single small JS file that works from a `<script>` tag or as an ES module.

- **Demos:** [overview](https://diegoicosta.github.io/richman-gallery/demo/) · [all features](https://diegoicosta.github.io/richman-gallery/demo/all-features.html)
- **Technical manual:** [docs/technical-manual.md](docs/technical-manual.md) — implementation details, API and callbacks
- **License:** [MIT](LICENSE)
...
```html
<link rel="stylesheet" href="richman-gallery.css">
...
the images are laid out in a masonry grid (CSS columns), and clicking one opens the fullscreen viewer.
...
A bar appears at the bottom of the gallery while at least one image is selected. It shows:
...
Each tag is rendered as its own element, so a user can style them individually (see the [technical manual](docs/technical-manual.md)). Tags are added automatically for any image that declares them, including images inserted into the gallery after the page has already loaded.
...
Requires Node. Install dependencies, then:

```sh
npm run build       # bundles to dist/ (ESM + IIFE) and type declarations
npm run typecheck   # tsc --noEmit
npm test            # vitest (jsdom)
```

The behavioral contract is the [technical manual](docs/technical-manual.md); the demos in `demo/` exercise every feature and are published to GitHub Pages by CI.
